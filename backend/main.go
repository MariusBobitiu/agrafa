...
func main() {
	...
	projectMemberRepo := repositories.NewProjectMemberRepository(dbConn, queries)
	...
	nodeRepo := repositories.NewNodeRepository(dbConn, queries)
	serviceRepo := repositories.NewServiceRepository(dbConn, queries)
	heartbeatRepo := repositories.NewHeartbeatRepository(queries)
	healthCheckRepo := repositories.NewHealthCheckRepository(dbConn, queries)
	metricRepo := repositories.NewMetricRepository(dbConn, queries)
	eventRepo := repositories.NewEventRepository(queries)
	overviewRepo := repositories.NewOverviewRepository(dbConn, queries)
	alertRuleRepo := repositories.NewAlertRuleRepository(dbConn, queries)
	alertInstanceRepo := repositories.NewAlertInstanceRepository(dbConn, queries)
	notificationRecipientRepo := repositories.NewNotificationRecipientRepository(dbConn, queries)
