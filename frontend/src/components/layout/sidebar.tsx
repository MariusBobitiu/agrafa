...
const NAV_ITEMS = [
  ...
  { to: "/nodes", label: "Nodes", icon: CircuitBoardIcon },
    ...
    >
      ...
      <div className="px-2 pb-2">
        <Tooltip delayDuration={0}>
          <TooltipTrigger asChild>
            <AnimateIcon asChild animate={isRouteActive("/instance-settings")} animateOnHover>
              <Link
                to="/instance-settings"
                aria-current={isRouteActive("/instance-settings") ? "page" : undefined}
                data-active={isRouteActive("/instance-settings") ? "true" : "false"}
                className={cn(
                  "flex items-center gap-3 rounded-md px-2 py-2 text-sm font-medium transition-colors",
                  isRouteActive("/instance-settings")
                    ? "bg-sidebar-primary text-sidebar-primary-foreground"
                    : "text-muted-foreground hover:bg-sidebar-primary/50 hover:text-foreground",
                  sidebarOpen ? "justify-start px-4" : "justify-center px-2",
                )}
              >
                <CogIcon size={16} strokeWidth={2.5} className="shrink-0" />
                {sidebarOpen && (
                  <motion.span
                    animate={{ opacity: 1 }}
                    initial={{ opacity: 0 }}
      ...
      <div className="px-2 py-2">
        <AnimateIcon asChild animateOnHover>
          ...
          >
            <span className={cn(!sidebarOpen && "sr-only")}>Toggle sidebar</span>
