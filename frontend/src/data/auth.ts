...
export const authApi = {
  ...
  login: (input: LoginInput): Promise<AuthSessionResponse> => api.post("/auth/login", input),
  ...
  completeOnboarding: (): Promise<{ user: User }> => api.post("/auth/onboarding/complete"),

  sendVerifyEmail: (): Promise<AuthStatusResponse> => api.post("/auth/verify-email/send"),
  ...
  listSessions: (): Promise<{ sessions: AuthSession[] }> => api.get("/auth/sessions"),

  logoutAll: (): Promise<AuthStatusResponse> => api.post("/auth/logout-all"),

  deleteSession: (id: string): Promise<void> => api.del(`/auth/sessions/${id}`),

  changePassword: (input: {
    current_password: string;
    new_password: string;
  }): Promise<AuthStatusResponse> => api.patch("/auth/password", input),
};
