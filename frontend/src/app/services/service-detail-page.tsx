...
import { useService } from "@/hooks/use-services.ts";
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 min-w-0">
      ...
      <div className="flex items-center gap-3 ml-2 shrink-0 text-xs text-muted-foreground tabular-nums">
        ...
        <span>
          {check.status_code != null ? `HTTP ${check.status_code}` : "No response"}
        </span>
      ...
      <div className="mx-auto max-w-6xl space-y-7">
        ...
        <div className="rounded-xl border border-border bg-card p-6 space-y-4">
          ...
          <div className="pt-4 border-t border-border flex gap-5">
            {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-3.5 w-20" />)}
          </div>
        </div>
        <div className="space-y-4">
          <div className="flex gap-2">
            {Array.from({ length: 12 }).map((_, i) => <Skeleton key={i} className="h-4 w-4 rounded-full" />)}
          </div>
...
export function ServiceDetailPage() {
  ...
  const isDegraded = latest ? !latest.is_success && service.status === "degraded" : service.status === "degraded";
			...
			<div className="mx-auto max-w-6xl space-y-7">
							...
							<MetaItem
								...
								value={
									service.last_checked_at
										? formatRelativeTime(service.last_checked_at)
										: "Never"
								}
							/>
				...
				<section>
					...
					{alerts.length === 0 ? (
						<div className="flex items-center gap-2 py-1">
							...
							<p className="text-sm text-muted-foreground">
								No active alerts on this service.
							</p>
						</div>
					) : (
				...
				<section>
					...
					<div className="flex flex-wrap gap-x-6 gap-y-2">
						<MetaItem
							label="Check type"
							value={service.check_type.toUpperCase()}
						/>
