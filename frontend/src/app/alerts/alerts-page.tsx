...
function AlertHistoryTable({ alerts }: { alerts: Alert[] }) {
	const columns: ColumnDef<Alert>[] = [
		...
		},
		{
			...
			meta: {
				...
				cellClassName:
					"hidden px-4 py-3 text-xs text-muted-foreground tabular-nums sm:table-cell",
			},
		...
		},
		{
			...
			meta: {
				...
				cellClassName:
					"hidden px-4 py-3 text-xs text-muted-foreground tabular-nums md:table-cell",
			},
			cell: ({ row }) =>
				row.original.resolved_at
					? formatRelativeTime(row.original.resolved_at)
					: "—",
		},
		{
			id: "status",
	...
	return (
		<DataTable
			...
			rowClassName={(row) =>
				row.original.status === "resolved" ? "opacity-50" : undefined
			}
		/>
	);
}
