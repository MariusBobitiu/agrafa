...
const quietTrigger =
	"h-9 text-sm focus:ring-0 focus:ring-offset-0 focus:border-border";
...
function Hint({ children }: { children: React.ReactNode }) {
	return (
		<p className="text-xs text-muted-foreground/70 leading-snug">{children}</p>
	);
}
	...
	async function onSubmit(values: FormValues) {
		try {
			await createRule.mutateAsync({
				...
				node_id:
					values.nodeId && values.nodeId !== "__any__"
						? Number(values.nodeId)
						: null,
				service_id:
					values.serviceId && values.serviceId !== "__any__"
					...
					<form onSubmit={form.handleSubmit(onSubmit)}>
						<div className="space-y-3 px-5 py-4">
							...
							{rule?.conditionType === "threshold" && (
								<FormField
									...
									render={({ field }) => (
										<FormItem className="gap-0">
											<FormLabel className="mb-1.5 text-sm">
												Alert when above
											</FormLabel>
						...
						<div className="flex items-center justify-end gap-2 border-t border-border/60 px-5 py-3">
							...
							<Button
								type="submit"
								size="sm"
								disabled={createRule.isPending || !selectedRuleType}
							>
