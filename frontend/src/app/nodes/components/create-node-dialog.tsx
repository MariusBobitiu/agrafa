...
const STEP_COPY = [
  ...
  {
    title: "Install agent",
    description: "Run the agent container on the target machine using the generated token.",
  },
  {
    title: "Wait for heartbeat",
    description: "Agrafa will mark the node online as soon as it receives the first heartbeat.",
  },
] as const;
...
export function CreateNodeDialog({
  ...
  async function handleCreate(values: FormValues) {
    try {
      ...
      try {
        const tokenResponse = await regenerateToken.mutateAsync(response.node.id);
        ...
        <div className="flex-1 space-y-3 overflow-x-hidden overflow-y-auto px-6 pb-2">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center">
                  ...
                  <p className="text-sm text-foreground">After running this, come back here</p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Make sure the container is running and can reach the internet. If it still
                  doesn&apos;t connect, regenerate the token and restart the agent.
                </p>
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    {regenerateToken.isPending ? "Generating new token..." : "Regenerate token"}
                  </Button>
          ...
          {step === 2 && createdNode && (
            <div className="space-y-6">
              ...
              {createdNode ? (
                <div className="pt-1">
                  <p className="text-sm text-foreground mb-2">Node status</p>
                  <NodeSnapshot
                    node={createdNode}
              ...
              {nodeQuery.isError ? (
                ...
                <Alert className="border-primary/20 bg-primary/5">
                  ...
                  <AlertDescription>
                    If the container is already running, refresh manually or wait for the next
                    automatic poll.
                  </AlertDescription>
                </Alert>
              )}
        ...
        <DialogFooter className="border-t border-border/70 bg-muted/10 px-6 py-5">
          ...
          {step === 0 && (
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" form="create-node-form" disabled={createNode.isPending}>
            ...
            <div className="w-full flex flex-wrap justify-between gap-2">
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  disabled={regenerateToken.isPending}
                >
                  {regenerateToken.isPending ? "Generating..." : "Regenerate token"}
                </Button>
              </div>
              <Button
                type="button"
                onClick={handleFinish}
                disabled={!isNodeOnline || regenerateToken.isPending || !rawToken}
                className="min-w-32"
              >
